...
setwd("~/Documentos/FCiencias/2021-1/BEDU/Fase_2/Sesion_8/Postwork_8/")
...
png("www/plot_1.png")
p1 <- ggplot(g, aes(x=Num_Ap, y=Capital)) + geom_line( color="purple") + geom_point() +
...
g <- data.frame(Num_Ap = 1:length(g), Capital = g)
png("www/plot_2.png")
p2 <- ggplot(g, aes(x=Num_Ap, y=Capital)) + geom_line( color="purple") + geom_point() +
    labs(x = "Número de Apuesta", 
         y = "Capital",
         title = "Realizando una secuencia de apuestas") +
    theme(plot.title = element_text(size=12))  +
